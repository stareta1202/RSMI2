...
vector<Point> FileReader::get_tweets(double *min_x, double *max_x, double *min_y, double *max_y)
{
    ifstream file(filename);
    vector<Point> points;
    string line = "";
    while (getline(file, line))
    {
        ...
        if (vec.size() > 1)
        {
            if (stod(vec[1]) < (*min_x)) (*min_x) = stod(vec[1]);
            if (stod(vec[1]) > (*max_x)) (*max_x) = stod(vec[1]);
            if (stod(vec[2]) < (*min_y)) (*min_y) = stod(vec[2]);
            if (stod(vec[2]) > (*max_y)) (*max_y) = stod(vec[2]);
            Point point(stod(vec[1]), stod(vec[2]));
            points.push_back(point);
        }
    }
    // Close the File
